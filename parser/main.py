...
async def save_message(conn: asyncpg.Connection, msg, sender_uuid: uuid.UUID) -> None:
    ...
    await conn.execute(
        SQL_INSERT_MESSAGE, chat_id, msg_id, send_time, sender_uuid, mtype, text
    )
            ...
            try:
                ...
                if isinstance(group, int):
                    try:
                        ...
                        print(
                            f"Resolved group by trying '-100' prefix: using {alt} as the entity."
                        )
                    except ValueError:
                        print(
                            f"Failed to resolve numeric group id {group} even after trying -100 prefix."
                        )
                        print(
                            "Ensure the session account is a member of the group or provide the group's @username instead."
                        )
                        continue
                else:
            ...
            if chat_id is None:
                try:
                    ...
                    if ent_id is not None:
                        ...
                        group_name = (
                            getattr(resolved_ent, "title", None)
                            or getattr(resolved_ent, "username", None)
                            or str(group)
                        )
                except Exception:
                    chat_id = None
            ...
            if chat_id is not None:
                try:
                    row = await conn.fetchrow(
                        "SELECT max(message_id) AS last_id FROM messages WHERE chat_id=$1",
                        chat_id,
                    )
                    if row:
                        saved_last_id = row.get("last_id")
                        print(
                            f"Last saved message id for chat_id {chat_id} is {saved_last_id}"
                        )
                except Exception as e:
            ...
            try:
                if saved_last_id is not None and sample:
                    ...
                    if latest is not None and latest <= saved_last_id:
                        print(
                            f"No new messages since last_id {saved_last_id} for chat_id {chat_id}; skipping."
                        )
                        continue
            except Exception:
                pass
            ...
            try:
                if saved_last_id:
                    ...
                    async for msg in client.iter_messages(
                        entity, reverse=True, min_id=saved_last_id
                    ):
                        # skip already saved messages if DB indicates so (best-effort)
                        if (
                            getattr(msg, "id", None) is not None
                            and msg.id <= saved_last_id
                        ):
                            continue
                        ...
                        if counter % progress_every == 0:
                            ...
                            print(
                                f"Processed {counter} messages for {group} (last id={msg.id}, date={last_dt})"
                            )
                else:
                ...
                print(
                    f"Finished processing history for {group}. New messages processed: {counter}"
                )

    print(
        f"Finished full history sync. Total new messages processed across groups: {total_counter}"
    )
...
async def main():
    ...
    if client is None:
        proxy = None
        if SOCKS5_PROXY_HOST and SOCKS5_PROXY_PORT > 0:
            proxy = (
                socks.SOCKS5,
                SOCKS5_PROXY_HOST,
                SOCKS5_PROXY_PORT,
            ...
            print(f"SOCKS5 proxy enabled: {SOCKS5_PROXY_HOST}:{SOCKS5_PROXY_PORT}")
